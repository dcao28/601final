...
# load("../data/set_b_t.RData")#y=TP53
# load("../data/set_c_t.RData")#y=TP53
# load("../data/set_combine_t.RData")#y=TP53
cat("read 6 data sets \n")
### link function G1：4 ####  
g1<-function(x){
  return(x)
}
...
findopt1 <- function(lambda,g,model){#with linear assumption
  ...
  fn<-function(beta){
    ...
    vg<-var(fit.n)
    return(-(vg/(vg+var(resid)))+ lambda1*var(fit.n,resid)+lambda2*sum(abs(beta)))
  }
