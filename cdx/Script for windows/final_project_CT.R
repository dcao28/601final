...
lambda2<- seq(0,3,by = 0.5)#lasso lambda range
lambda1<- 1
...
### forward.gmc set_c_t G1 ####
addFormula <- function(add1="") {
  text <- paste("~ .",paste("+", add1, collapse = " "))
  eval(parse(text = text))
}

cat("begin gmc forward selection in Set.C.T with G1\n")
pre.m <- lm(TP53 ~ 1, data = set_c_t)
...
for(i in seq(10)){# step of forward selection
  ...
  if(is.null(pre.xy)){
    avail.x <- setdiff(names(set_c_t),"TP53")
  }else{avail.x <- setdiff(names(set_c_t),pre.xy)}
...
save(stepBestGLM,file = "../Rdata/GMC/stepBestGlM_CT_G1.RData")

### forward.gmc set_c_t G2 ####
addFormula <- function(add1="") {
  text <- paste("~ .",paste("+", add1, collapse = " "))
  eval(parse(text = text))
}

cat("begin gmc forward selection in Set.C.T with G2\n")
pre.m <- lm(TP53 ~ 1, data = set_c_t)
stepBestGLM<- vector(mode="list")
stepBestLM <- vector(mode="list")
gmcs <- c()
...
cat("time cost",end-start,"minites")
save(stepBestGLM,file = "../Rdata/GMC/stepBestGlM_CT_G2.RData")

### forward.gmc set_c_t G3 ####
addFormula <- function(add1="") {
  text <- paste("~ .",paste("+", add1, collapse = " "))
  eval(parse(text = text))
}

cat("begin gmc forward selection in Set.C.T with G3\n")
pre.m <- lm(TP53 ~ 1, data = set_c_t)
stepBestGLM<- vector(mode="list")
stepBestLM <- vector(mode="list")
gmcs <- c()
...
cat("time cost",end-start,"minites")
save(stepBestGLM,file = "../Rdata/GMC/stepBestGlM_CT_G3.RData")

### forward.gmc set_c_t G4 ####
addFormula <- function(add1="") {
  text <- paste("~ .",paste("+", add1, collapse = " "))
  eval(parse(text = text))
}

cat("begin gmc forward selection in Set.C.T with G4\n")
pre.m <- lm(TP53 ~ 1, data = set_c_t)
stepBestGLM<- vector(mode="list")
stepBestLM <- vector(mode="list")
gmcs <- c()
...
for(i in seq(10)){# step of forward selection
  ...
  #compute_GMC(new.m[[1]],g = g4,findopt = findopt1)
...
cat("time cost",end-start,"minites")
save(stepBestGLM,file = "../Rdata/GMC/stepBestGlM_CT_G4.RData")
