...
### forward.gmc set_c_log G1 ####
addFormula <- function(add1="") {
  text <- paste("~ .",paste("+", add1, collapse = " "))
  eval(parse(text = text))
}

cat("begin gmc forward selection in Set.C.Log with G1\n")
pre.m <- lm(y ~ 1, data = set_c_log)
...
for(i in seq(10)){# step of forward selection
  ...
  if(is.null(pre.xy)){
    avail.x <- setdiff(names(set_c_log),"y")
  }else{avail.x <- setdiff(names(set_c_log),pre.xy)}
...
save(stepBestGLM,file = "../Rdata/GMC/stepBestGlM_Clog_G1.RData")

### forward.gmc set_c_log G2 ####
addFormula <- function(add1="") {
  text <- paste("~ .",paste("+", add1, collapse = " "))
  eval(parse(text = text))
}

cat("begin gmc forward selection in Set.C.Log with G2\n")
pre.m <- lm(y ~ 1, data = set_c_log)
stepBestGLM<- vector(mode="list")
stepBestLM <- vector(mode="list")
gmcs <- c()
...
cat("time cost",end-start,"minites")
save(stepBestGLM,file = "../Rdata/GMC/stepBestGlM_Clog_G2.RData")

### forward.gmc set_c_log G3 ####
addFormula <- function(add1="") {
  text <- paste("~ .",paste("+", add1, collapse = " "))
  eval(parse(text = text))
}

cat("begin gmc forward selection in Set.C.Log with G3\n")
pre.m <- lm(y ~ 1, data = set_c_log)
stepBestGLM<- vector(mode="list")
stepBestLM <- vector(mode="list")
gmcs <- c()
...
cat("time cost",end-start,"minites")
save(stepBestGLM,file = "../Rdata/GMC/stepBestGlM_Clog_G3.RData")

### forward.gmc set_c_log G4 ####
addFormula <- function(add1="") {
  text <- paste("~ .",paste("+", add1, collapse = " "))
  eval(parse(text = text))
}

cat("begin gmc forward selection in Set.C.Log with G4\n")
pre.m <- lm(y ~ 1, data = set_c_log)
stepBestGLM<- vector(mode="list")
stepBestLM <- vector(mode="list")
gmcs <- c()
...
cat("time cost",end-start,"minites")
save(stepBestGLM,file = "../Rdata/GMC/stepBestGlM_Clog_G4.RData")
