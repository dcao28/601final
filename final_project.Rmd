...
OptEveryLambda<-mclapply(lambda.set, FUN = findopt,mc.cores=2)
colnames(OpetEveryLambda)<- c("lambda","","GMC(Y|G(X))")
...
findopt<-function(lambda){
  ...
  return(-GMC(data)[2]+ lambda*sum(abs(beta)))#(GMC(X|Y),GMC(Y|X))
  }
...
OptEveryLambda<-t() mclapply(lambda.set, FUN = findopt,mc.cores=detectCores())
colnames(OpetEveryLambda)<- c("lambda","","GMC(Y|G(X))")
# fn<-function(beta){
#   xb= as.matrix(bestmodel$model[-1]) %*% beta[-1]+beta[1]
#   
